                              ...
                              UINT32 nClientPort)   //  [IN] Client side port 0 for none
{
    ...
    do
    {
        ...
        if(pHTTPSession->HttpFlags & HTTP_CLIENT_FLAG_JSON)
		{
        	if((nRetCode = HTTPIntrnHeadersAdd(pHTTPSession,"Content-Type",12,"application/json",16))!= HTTP_CLIENT_SUCCESS)
		   {
				break;
		   }
		}
        ...
        {
          if((nRetCode = HTTPIntrnHeadersAdd(pHTTPSession,"Content-Type",12,"application/octet-stream",24))!= HTTP_CLIENT_SUCCESS)
		   {
				break;
		   }
		}
            ...
            pHTTPSession->HttpProxy.ProxyAuthSchema != AuthSchemaNone) // If we have to authenticate we should use the HEAD verb
        {
            ...
            else
            {
                ...
                if(((pHTTPSession->HttpHeaders.HttpVerb == VerbPut) || (pHTTPSession->HttpHeaders.HttpVerb == VerbPost)) &&  TotalLength == TRUE)
                {
