...
#define HTTP_CLIENT_FLAG_JSON               0x00000008 // content type json
...
#define HTTP_CLIENT_ERROR_SERVER_ERROR       31 // Feature is not (yet) implemented
...
#define HTTP_CLIENT_EOS                     1000        // HTTP end of stream message
    ...
    {	 
        unsigned int        HTTPStatusCode;                 // HTTP Status code (200 OK)
		unsigned int		RequestBodyLengthSent;          // Total bytes sent (body only)
		unsigned int		ResponseBodyLengthReceived;     // Total bytes received (body only)
		unsigned int		TotalResponseBodyLength;        // as extracted from the �content-length" header
		unsigned int        HttpState;
    } HTTP_CLIENT;
